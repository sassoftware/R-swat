...
test_that("tail", {
  ...
  ct@orderby <- "n1"
  expect_equivalent(tail(df[order(df$n1), c(1:4)], 4), tail(ct[c(1:4)], 4))
  expect_true(all.equal(tail(df[order(df$n1), c(1:4)], 4), tail(ct[c(1:4)], 4), tolerance=1e-06, check.attributes=FALSE))
