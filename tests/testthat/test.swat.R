...
test_that("test.test_summary", {
  ...
  expect_true(t[[4]] == "numeric")
  expect_true(t[[5]] == "numeric")
...
test_that("test.test_upload", {
  caz$upload(iris, casout = list(name = "iris2", replace = TRUE))
  ...
  # No casout
  out <- caz$upload(test.data("effort_cas.xlsx"))
  expect_equivalent(out$results$tableName, "EFFORT_CAS")

  # Missing file
  expect_error(caz$upload("/path/to/missing/file.txt", casout = list(name = "missing", replace = TRUE)))
})

test_that("loadactionset", {
  if (exists("cas.neuralNet.annCode")) {
    skip("neuralNet is already loaded")
  }

  info <- cas.retrieve(caz, "builtins.actionsetinfo", all = FALSE)$results$setinfo
  expect_true(nrow(info) > 0)
  sets <- tolower(info$actionset)
  expect_false("neuralnet" %in% sets)
  expect_false(exists("cas.neuralNet.annCode"))

  res <- cas.retrieve(caz, "builtins.loadactionset", actionset = "neuralnet")
  expect_equivalent(res$disposition$severity, 0)

  info <- cas.retrieve(caz, "builtins.actionsetinfo", all = FALSE)$results$setinfo
  expect_true(nrow(info) > 0)
  sets <- tolower(info$actionset)
  expect_true("neuralnet" %in% sets)
  expect_true(exists("cas.neuralNet.annCode"))
})

test_that("cas.upload.file (url)", {
  tbl <- cas.upload.file(caz, "http://s3.amazonaws.com/assets.datacamp.com/course/Kaggle/train.csv")
  expect_that(tbl, is_a("CASTable"))
  expect_equivalent(names(tbl), names(titanic))
  expect_equivalent(nrow(tbl), nrow(titanic))
})

test_that("cas.upload.frame", {
  tbl <- cas.upload.frame(caz, iris)
  expect_that(tbl, is_a("CASTable"))
  expect_equivalent(names(tbl), names(iris))
  expect_equivalent(nrow(tbl), nrow(iris))
})

test_that("cas.copy", {
  caz2 <- cas.copy(caz)
  expect_equivalent(caz2$soptions, caz$soptions)
  expect_equivalent(caz2$hostname, caz$hostname)
  expect_equivalent(caz2$port, caz$port)
  expect_equivalent(caz2$protocol, caz$protocol)
  expect_equivalent(caz2$username, caz$username)
  expect_true(caz2$session != caz$session)
  cas.close(caz2, TRUE)
})

test_that("disposition", {
  out <- cas.simple.summary(caz)
  expect_true(grepl('[\\w+-]+\\w+', cas.session(caz)))
  expect_true(length(cas.messages(caz)) > 0)
  expect_equivalent(cas.status(caz), "Error parsing action parameters.")
  expect_equivalent(cas.status_code(caz), 2720411)
  expect_equivalent(cas.debug(caz), "0x88bfc19b:TKCASA_GEN_PARSE_ERROR")
  expect_equivalent(cas.reason(caz), "abort")
  expect_equivalent(cas.severity(caz), 2)
  expect_true(length(names(cas.performance(caz))) > 0)
  expect_that(cas.events(caz), is_a("list"))

  out <- cas.simple.summary(t, foo = "bar")
  expect_true(grepl('[\\w+-]+\\w+', cas.session(t)))
  expect_true(length(cas.messages(t)) > 0)
  expect_equivalent(cas.status(t), "Error parsing action parameters.")
  expect_equivalent(cas.status_code(t), 2720411)
  expect_equivalent(cas.debug(t), "0x88bfc19b:TKCASA_GEN_PARSE_ERROR")
  expect_equivalent(cas.reason(t), "abort")
  expect_equivalent(cas.severity(t), 2)
  expect_true(length(names(cas.performance(t))) > 0)
  expect_that(cas.events(t), is_a("list"))

  out <- caz$retrieve("simple.summary")
  expect_true(length(cas.messages(out)) > 0)
  expect_equivalent(cas.status(out), "Error parsing action parameters.")
  expect_equivalent(cas.status_code(out), 2720411)
  expect_equivalent(cas.debug(out), "0x88bfc19b:TKCASA_GEN_PARSE_ERROR")
  expect_equivalent(cas.reason(out), "abort")
  expect_equivalent(cas.severity(out), 2)
  expect_true(length(names(cas.performance(out))) > 0)
  expect_that(cas.events(out), is_a("list"))
})
