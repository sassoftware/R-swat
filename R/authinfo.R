...
.query_authinfo <- function(hostname, username = NULL, protocol = NULL, filepath = NULL) {
  ...
  authinfo_paths <- c(
    "_authinfo.gpg", ".authinfo.gpg", "_netrc.gpg", ".netrc.gpg",
  ...
  if (is.null(filepath)) {
    if (nchar(Sys.getenv("AUTHINFO")) > 0) {
      ...
      for (i in seq_len(length(authinfo_paths))) {
  ...
  for (i in seq_len(length(authinfo_paths))) {
    ...
    info <- scan(authinfo_paths[[i]], character(0),
      comment.char = "#",
